...
//        logger.info(gson.toJson(list));
	...
	public static CellStyle getCellStyle(CellStyle cellstytle) {
		

		// 垂直居中
		cellstytle.setVerticalAlignment(CellStyle.VERTICAL_CENTER);
		cellstytle.setAlignment(CellStyle.ALIGN_CENTER);
	...
	public static int writeExcel_ex(String filepath, List<String> tablenames, List<List<Column>> columns) throws IOException {
		...
		/* 0.设置单元格样式 */
		// 设置列宽
		sheet.setColumnWidth(0, 256 * 30);// 表名
		sheet.setColumnWidth(1, 256 * 25);// 表描述
		sheet.setColumnWidth(2, 256 * 20);// 列名
		sheet.setColumnWidth(3, 256 * 15);// 字段类型
		sheet.setColumnWidth(4, 256 * 10);// 长度
		...
		// 合并的单元格样式
		CellStyle cellstytle = workbook.createCellStyle();
		// 垂直居中
		cellstytle.setVerticalAlignment(CellStyle.VERTICAL_CENTER);
		cellstytle.setAlignment(CellStyle.ALIGN_CENTER);
		cellstytle.setFont(font);
		// 设置边框
		cellstytle.setBorderBottom(CellStyle.BORDER_THIN); // 下边框
	...
	public static int writeExcel(List<String[]> message, String path) {
		...
		/* 1.先将内容写一遍 */
		writeExcel(message, workbook, sheet, cellstytle);
	...
	/* 按照格式输出文件左右文件 */
	public static void writeEXCEL(List<String[]> message, String path, List<String> tablenames,
	...
	/* 负责写左边的内容 */
	public static void writeExcel(List<String[]> message, XSSFWorkbook workbook, XSSFSheet sheet,
			CellStyle cellstytle) {
...
//				start = i;
	...
	/* 关于二级联动下拉框的创建 */
	public static void Cascade(XSSFWorkbook book, XSSFSheet sheetPro, List<String> tablenames,
			List<List<Column>> columns, CellStyle cellStyle) {
